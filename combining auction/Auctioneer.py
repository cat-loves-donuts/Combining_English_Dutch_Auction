...
class Auctioneer():
    def __init__(self, Msg_type, Auction_ID,Auctioneer_ID, Good_ID, Good_name, Reserve, time, amount_of_bidders,Increase,Derease,Dutch_Increase):
        ...
        self.Derease = Derease
        self.Dutch_Increase = Dutch_Increase
        self.Reserve = Reserve
        self.Initialprice = Reserve * self.Increase
        self.time = time
        self.offer = 0
    ...
    def English_process_Msg(self, tick):
        ...
        if count == 0:
            ...
            accepter = self.winner[0][3]                                         # 非随机在所有胜利者中的一个，后面来改
            self.offer = offer
            for i in range(0,self.number):
    ...
    def Dutch_process_Msg(self,tick):
        ...
        if (tick > (self.time-11)) & (tick < self.time):
            if len(self.accept) == 1:
                ...
                if self.accept[0][6] >= self.Reserve:
                    ...
                    for i in range(0, self.number):
                        ...
                        self.new_Msg[i][9] = 0
            if len(self.accept) > 1:
                self.Initialprice = (self.Initialprice / self.Derease - 10)
            ...
            if len(self.accept) == 0:
                self.Initialprice = self.Initialprice * self.Derease
